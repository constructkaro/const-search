...
<style>
    ...
    .audit-page{ max-width:1220px; margin:0 auto; padding:22px 0 36px; }
    .audit-stack{ display:flex; flex-direction:column; gap:24px; }
    ...
    .top-banner-icon{
        ...
        font-size:26px; box-shadow:0 16px 32px rgba(235,122,47,0.22); flex-shrink:0;
    }
    .top-banner h1{ margin:0; color:var(--ck-navy); font-size:28px; line-height:1.2; font-weight:900; }
    .top-banner p{ margin:6px 0 0; color:var(--ck-text-soft); font-size:14px; font-weight:500; }
    .top-badge{
        padding:11px 16px; border-radius:999px; background:#fff; border:1px solid #f1dfcf;
        color:var(--ck-orange); font-size:14px; font-weight:800; box-shadow:0 8px 18px rgba(235,122,47,0.06);
    }
    ...
    .step-head{ display:flex; align-items:flex-start; gap:16px; margin-bottom:24px; position:relative; z-index:2; }
    .step-badge{
        width:54px; height:54px; border-radius:18px;
        background: linear-gradient(135deg, #fff4eb 0%, #ffe8d8 100%);
        color:var(--ck-orange); display:flex; align-items:center; justify-content:center;
        font-size:22px; flex-shrink:0;
    }
    .step-title-wrap h2{ margin:0; font-size:24px; line-height:1.15; color:var(--ck-navy-2); font-weight:900; }
    .step-title-wrap p{ margin:6px 0 0; color:var(--ck-text-soft); font-size:15px; font-weight:500; }

    /* ── Field ── */
    .field-block + .field-block{ margin-top:24px; }
    .field-label{ margin:0 0 10px; font-size:16px; color:var(--ck-navy); font-weight:800; }
    .field-label .req{ color:var(--ck-red); }
    .field-help, .field-sub{ margin:-2px 0 14px; font-size:14px; color:var(--ck-text-soft); font-weight:500; }

    /* ── Vendor bar ── */
    .vendor-bar{
        display:flex; align-items:center; justify-content:space-between; gap:16px;
        padding:16px 18px; border:1.5px solid var(--ck-line-dark);
        border-radius:var(--ck-radius-md);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
    }
    .vendor-value{ font-size:16px; font-weight:800; color:var(--ck-navy); }
    .vendor-chip{
        background:linear-gradient(135deg,var(--ck-orange) 0%,var(--ck-orange-2) 100%);
        color:#fff; padding:9px 14px; border-radius:999px; font-size:13px; font-weight:800; white-space:nowrap;
    }

    /* ── Project / Asset grid (checkbox rows) ── */
    .project-grid{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:16px 18px; }
    .project-item input,
    .card-option input,
    .chip-option input,
    .toggle-switch input,
    ...
    .project-item label{
        min-height:64px; border:1.5px solid var(--ck-line-dark);
        border-radius:var(--ck-radius-md);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
        display:flex; align-items:center; gap:14px; padding:14px 16px;
        cursor:pointer; transition:all .22s ease; font-size:15px; color:var(--ck-text); font-weight:700; line-height:1.4;
    }
    .project-item label::before{
        content:""; width:20px; height:20px; border-radius:6px;
        border:2px solid #c4d0de; background:#fff; flex-shrink:0; transition:.22s ease;
    }
    .project-item label:hover{ transform:translateY(-2px); box-shadow:var(--ck-shadow-sm); border-color:#c7d3e4; }
    .project-item input:checked + label{ border-color:var(--ck-orange); background:linear-gradient(180deg,#fffaf6 0%,#fff2e9 100%); box-shadow:0 10px 18px rgba(235,122,47,0.08); }
    ...
    /* ── Structure card grid ── */
    .structure-grid{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:18px; max-width:820px; }
    .card-option label{
        min-height:142px; border:1.5px solid var(--ck-line-dark);
        border-radius:var(--ck-radius-lg);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
        display:flex; flex-direction:column; align-items:center; justify-content:center;
        gap:13px; text-align:center; padding:18px 14px;
    ...
    .card-option label:hover{ transform:translateY(-4px); box-shadow:var(--ck-shadow-sm); border-color:#c7d3e4; }
    .card-option label:hover::after{ opacity:1; }
    .card-icon{
        width:62px; height:62px; border-radius:18px; background:#f2f6fc;
        color:var(--ck-navy-3); display:flex; align-items:center; justify-content:center;
        font-size:28px; position:relative; z-index:2; transition:all .24s ease;
    }
    .card-text{ font-size:17px; line-height:1.35; color:var(--ck-navy-2); font-weight:800; position:relative; z-index:2; transition:all .24s ease; }
    .card-option input:checked + label{ border-color:var(--ck-orange); background:linear-gradient(180deg,#fffaf6 0%,#fff2e9 100%); box-shadow:0 16px 28px rgba(235,122,47,0.10); }
    .card-option input:checked + label .card-icon{ background:#ffe7d6; color:var(--ck-orange); transform:scale(1.03); }
    .card-option input:checked + label .card-text{ color:var(--ck-navy); }
    .card-option input:checked + label::before{
        content:"\f00c"; font-family:"Font Awesome 6 Free"; font-weight:900;
        position:absolute; top:12px; right:12px; width:24px; height:24px;
        border-radius:50%; background:var(--ck-orange); color:#fff;
        display:flex; align-items:center; justify-content:center;
        font-size:9px; z-index:3; box-shadow:0 8px 14px rgba(235,122,47,0.18);
    }

    /* ── Chip grid ── */
    .chip-grid{ display:flex; flex-wrap:wrap; gap:12px; }
    .chip-option label{
        min-height:54px; padding:0 18px; border-radius:var(--ck-radius-sm);
        border:1.5px solid var(--ck-line-dark);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
        display:inline-flex; align-items:center; justify-content:center;
        gap:10px; cursor:pointer; font-size:15px; color:var(--ck-navy-2); font-weight:700; transition:all .22s ease;
    }
    .chip-option label:hover{ transform:translateY(-1px); box-shadow:var(--ck-shadow-sm); }
    .chip-option input:checked + label{
        border-color:var(--ck-orange);
        background:linear-gradient(180deg,#fffaf6 0%,#fff2e9 100%);
        color:var(--ck-navy); box-shadow:0 10px 18px rgba(235,122,47,0.08);
    }

    /* ── Form inputs ── */
    .form-grid-2{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:20px 24px; }
    .form-select, .form-input, .form-textarea{
        width:100%; border:1.5px solid var(--ck-line-dark);
        border-radius:var(--ck-radius-md); background:#fff; color:var(--ck-text);
        font-size:16px; padding:0 18px; outline:none; transition:all .22s ease;
        font-family: inherit;
    }
    .form-input::placeholder, .form-textarea::placeholder{ color:#9aa7b8; font-weight:500; }
    .form-select, .form-input{ height:60px; }
    .form-textarea{ min-height:170px; padding:18px; resize:vertical; }
    ...
    /* ── Toggle switch ── */
    .switch-panel{
        min-height:92px; border:1.5px solid var(--ck-line); border-radius:var(--ck-radius-lg);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
        display:flex; align-items:center; justify-content:space-between; gap:14px; padding:18px 20px;
        cursor:pointer; transition:all .22s ease;
    }
    .switch-panel:hover{ box-shadow:var(--ck-shadow-sm); border-color:#c6d3e4; }
    .switch-panel-title{ margin:0; font-size:16px; color:var(--ck-navy); font-weight:800; }
    .switch-panel-sub{ margin:4px 0 0; font-size:13px; color:var(--ck-text-soft); font-weight:500; }
    .switch-ui{
        width:70px; height:38px; border-radius:999px; background:#d8e0ec;
        position:relative; flex-shrink:0; transition:all .22s ease;
    }
    .switch-ui::before{
        content:""; position:absolute; top:4px; left:4px; width:30px; height:30px;
        border-radius:50%; background:#fff; box-shadow:0 4px 10px rgba(0,0,0,.10); transition:all .22s ease;
    }
    .toggle-switch input:checked + label .switch-ui{ background:#ffd7bf; }
    .toggle-switch input:checked + label .switch-ui::before{ left:36px; }

    /* ── Upload ── */
    .upload-grid{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:18px 24px; }
    .upload-title{ margin:0 0 10px; font-size:15px; color:var(--ck-navy); font-weight:800; }
    .upload-box{
        min-height:148px; border:1.8px dashed #c6d5e8; border-radius:var(--ck-radius-lg);
        background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);
        display:flex; flex-direction:column; align-items:center; justify-content:center;
        gap:9px; text-align:center; padding:16px; cursor:pointer; transition:all .22s ease;
    }
    .upload-box:hover{ transform:translateY(-2px); box-shadow:var(--ck-shadow-sm); border-color:#a9c0de; }
    .upload-box.active{ border-color:var(--ck-orange); background:linear-gradient(180deg,#fffaf6 0%,#fff2e9 100%); box-shadow:0 10px 18px rgba(235,122,47,0.08); }
    .upload-icon{ font-size:36px; color:#88a0c3; line-height:1; }
    .upload-text{ font-size:15px; color:var(--ck-navy-3); font-weight:500; }
    .file-name{ margin-top:8px; font-size:12px; font-weight:800; color:var(--ck-navy); word-break:break-word; }
    ...
    .submit-btn{
        min-width:350px; height:70px; border:none; border-radius:18px;
        background:linear-gradient(135deg,var(--ck-orange) 0%,var(--ck-orange-2) 100%);
        color:#fff; font-size:19px; font-weight:900;
        display:inline-flex; align-items:center; justify-content:center; gap:14px;
        box-shadow:var(--ck-shadow-lg); transition:all .22s ease; cursor:pointer;
    }
    .submit-btn:hover{ transform:translateY(-2px); box-shadow:0 22px 40px rgba(235,122,47,0.24); }
    .submit-note{ max-width:500px; color:var(--ck-text-soft); font-size:14px; font-weight:500; line-height:1.5; }

    /* ── Alerts & errors ── */
    .text-danger{ color:var(--ck-red); font-size:12px; margin-top:8px; display:block; font-weight:700; }
    .alert-success{ background:#ecfdf3; color:#027a48; border:1px solid #abefc6; padding:12px 14px; border-radius:14px; font-weight:700; }
    .alert-error{ background:#fef2f2; color:#b91c1c; border:1px solid #fecaca; padding:12px 14px; border-radius:14px; font-weight:700; }
    .error-list{ margin:0; padding-left:18px; }

    /* ── Responsive ── */
    @media(max-width:1100px){
        .project-grid, .upload-grid{ grid-template-columns:repeat(2,minmax(0,1fr)); }
    }
    ...
    @media(max-width:768px){
        .audit-page{ padding:12px 0 22px; }
        .top-banner{ padding:18px 16px; border-radius:20px; }
        .top-banner h1{ font-size:22px; }
        .step-card{ padding:18px 16px; border-radius:20px; }
        .step-title-wrap h2{ font-size:20px; }
        .vendor-bar{ flex-direction:column; align-items:flex-start; }
        .project-grid, .structure-grid, .upload-grid{ grid-template-columns:1fr; }
        .submit-btn{ width:100%; min-width:100%; height:60px; }
...
@php
    ...
    $oldStructureTypes  = old('structure_types',  []);
...
<div class="audit-stack">
    ...
    <div class="top-banner">
        <div class="top-banner-left">
            <div class="top-banner-icon"><i class="fa-solid fa-building-shield"></i></div>
            <div>
    ...
    <form action="{{ route('structural_audit.store') }}" method="POST" enctype="multipart/form-data" id="structuralAuditForm">
        ...
        {{-- ══════════════════════════════════════════════════
             STEP 1 · Asset Type Expertise
             DB column: asset_types  (longtext / JSON)
             Field name in form: asset_types[]
        ══════════════════════════════════════════════════ --}}
        <div class="step-card">
            <div class="step-head">
            ...
            {{--
                Project types from DB are used as ASSET TYPES here.
                name="asset_types[]"  →  controller validates asset_types
            --}}
            <div class="field-block">
                    ...
                    @forelse($projectTypes as $index => $type)
                        <div class="project-item">
                            <input
                                ...
                                name="asset_types[]"
                                value="{{ $type }}"
                                {{ in_array($type, $oldAssetTypes) ? 'checked' : '' }}
                            >
        ...
        {{-- ══════════════════════════════════════════════════
             STEP 2 · Structure Type Expertise
             DB column: structure_types  (longtext / JSON)
             Field name: structure_types[]
        ══════════════════════════════════════════════════ --}}
        <div class="step-card">
                ...
                @foreach($structureTypes as $index => $item)
                    <div class="card-option">
                        <input
                            ...
                            value="{{ $item['name'] }}"
                            {{ in_array($item['name'], $oldStructureTypes) ? 'checked' : '' }}
                        >
        ...
        {{-- ══════════════════════════════════════════════════
             STEP 3 · Audit Type Expertise
             DB column: audit_types  (longtext / JSON)
                ...
                @foreach($auditTypes as $index => $item)
                    <div class="chip-option">
                        <input
                            ...
                            name="audit_types[]"
                            value="{{ $item['name'] }}"
                            {{ in_array($item['name'], $oldAuditTypes) ? 'checked' : '' }}
                        >
        ...
        {{-- ══════════════════════════════════════════════════
             STEP 4 · Service Scope / Deliverables
             DB column: deliverables  (longtext / JSON)
                ...
                @foreach($deliverables as $index => $item)
                    <div class="chip-option">
                        <input
                            ...
                            name="deliverables[]"
                            value="{{ $item['name'] }}"
                            {{ in_array($item['name'], $oldDeliverables) ? 'checked' : '' }}
                        >
        ...
        {{-- ══════════════════════════════════════════════════
             STEP 5 · Technical & Business Information
        ══════════════════════════════════════════════════ --}}
        <div class="step-card">
            <div class="section-divider"></div>
            <div class="step-head">
            ...
            <div class="form-grid-2">
                ...
                <div class="field-block">
                    ...
                    <select class="form-select" id="team_size" name="team_size">
                        ...
                        @foreach(['1-5','6-10','11-25','26-50','50+'] as $opt)
                            <option value="{{ $opt }}" {{ old('team_size') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="field-block">
                    ...
                    <select class="form-select" id="minimum_project_value" name="minimum_project_value">
                        <option value="">Select project value</option>
                        @foreach(['Below 50K','50K - 1L','1L - 5L','5L - 10L','10L+'] as $opt)
                            <option value="{{ $opt }}" {{ old('minimum_project_value') == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                </div>

                {{-- Emergency Inspection toggle --}}
                <div class="field-block">
                    <div class="toggle-switch">
                        <input
                            ...
                            value="1"
                            {{ old('available_for_emergency_inspection') ? 'checked' : '' }}
                        >
                ...
                <div class="field-block">
                    <div class="toggle-switch">
                        <input
                            ...
                            value="1"
                            {{ old('available_for_site_visit') ? 'checked' : '' }}
                        >
                ...
                <div class="field-block">
                    ...
                    <input type="text" class="form-input" id="gst_number" name="gst_number"
                ...
                <div class="field-block">
                    ...
                    <input type="text" class="form-input" id="pan_number" name="pan_number"
                           placeholder="Enter PAN number" value="{{ old('pan_number') }}">
                    @error('pan_number')<small class="text-danger">{{ $message }}</small>@enderror
                </div>

                {{-- MSME toggle — full width --}}
                <div class="field-block" style="grid-column:1/-1;">
                    <div class="toggle-switch">
                        <input
                            ...
                            value="1"
                            {{ old('msme_udyam_registered') ? 'checked' : '' }}
                        >
            ...
            <div class="field-block">
                <div class="upload-grid">
                    ...
                    <div class="upload-item" id="certificate_wrap">
                        ...
                        @error('certificate_file')<small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                    ...
                    <div class="upload-item" id="company_profile_wrap">
                        ...
                        @error('company_profile_file')<small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                    ...
                    <div class="upload-item" id="logo_wrap">
                        ...
                        @error('logo_file')<small class="text-danger">{{ $message }}</small>@enderror
                    </div>

                </div>
            </div>

        </div>{{-- /step-card Technical --}}
        ...
        <div class="step-card">
            ...
            <div class="field-block">
                ...
                <textarea class="form-textarea" id="service_description" name="service_description"
                    placeholder="Describe your core structural audit services, specialisations, and key strengths...">{{ old('service_description') }}</textarea>
            ...
            <div class="field-block">
                ...
                <input type="text" class="form-input" id="major_cities_covered" name="major_cities_covered"
                       placeholder="e.g. Mumbai, Pune, Navi Mumbai, Thane"
                       value="{{ old('major_cities_covered') }}">
                @error('major_cities_covered')<small class="text-danger">{{ $message }}</small>@enderror
            </div>
        </div>
