...
<!-- OTP Modal -->
<div class="modal fade" id="loginRegisterModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:460px;">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header border-0 pb-0">
                <div class="w-100 text-center position-relative">
                    <h5 class="fw-bold mb-1" style="color:#1c2c3e;">Login Required</h5>
            ...
            <div class="modal-body px-4 pt-3 pb-4">
                <div class="mb-3">
                    <label class="fw-semibold mb-2" style="color:#1c2c3e;">Mobile Number</label>
                    <input type="text" id="otp_mobile" class="form-control form-control-lg" placeholder="Enter mobile number">
                </div>

                <div class="mb-3">
                    <button type="button" id="sendOtpBtn" class="btn w-100 text-white fw-bold"
                            style="background:linear-gradient(135deg,#2563eb,#1d4ed8); border-radius:14px; padding:12px 14px;">
                        Send OTP
                    </button>
                </div>

                <div class="mb-3">
                    <label class="fw-semibold mb-2" style="color:#1c2c3e;">Enter OTP</label>
                    <input type="text" id="otp_code" class="form-control form-control-lg" placeholder="Enter OTP">
                </div>

                <button type="button" id="verifyOtpBtn" class="btn w-100 text-white fw-bold"
                        style="background:linear-gradient(135deg,#ff9a3c,#f25c05); border-radius:14px; padding:12px 14px;">
                    Verify OTP & Continue
                </button>

                <div class="otp-note">
                    We’ll verify your number and continue your project submission.
                </div>
            </div>
        </div>
    </div>
            ...
            success: function (res) {
                ...
                if (res.status === 'payment_required') {
                    Swal.fire({
                        ...
                        text: res.message || 'Payment is required to continue.',
                ...
                if (res.status === 'success') {
                    Swal.fire({
                        ...
                        text: res.message || 'Your project has been submitted successfully.',
                        confirmButtonColor: '#f25c05'
                    });
    ...
    $('#verifyOtpBtn').on('click', function () {
        ...
        if (!mobile || !otp) {
            Swal.fire({
                icon: 'warning',
                title: 'Required',
                text: 'Please enter mobile number and OTP.',
                confirmButtonColor: '#f25c05'
            });
            return;
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                let modalInstance = bootstrap.Modal.getInstance(document.getElementById('loginRegisterModal'));
                ...
                if (res.status === true) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: res.post_saved ? 'Project Submitted' : 'Login Successful',
                        text: res.message,
                        confirmButtonColor: '#f25c05'
                    }).then(() => {
